...
public class MapperAnnotationBuilder {
	...
	private void parseCache() {
		...
		if (cacheDomain != null) {
			...
			Long flushInterval = cacheDomain.flushInterval() == 0 ? null
					: cacheDomain.flushInterval();
			assistant.useNewCache(cacheDomain.implementation(),
					cacheDomain.eviction(), flushInterval, size,
			...
			if (returnTypeParameter instanceof ParameterizedType) {
				...
				if (actualTypeArguments != null
					...
					if (returnTypeParameter instanceof Class) {
						...
						// (issue #443) actual type can be a also a
						// parameterized type
			...
			if (returnTypeParameter instanceof ParameterizedType) {
				...
				if (actualTypeArguments != null
					...
					if (returnTypeParameter instanceof Class) {
						...
						// (issue 443) actual type can be a also a parameterized
						// type
						returnType = (Class<?>) ((ParameterizedType) returnTypeParameter)
								.getRawType();
					}
		...
		for (Result result : results) {
			...
			ResultMapping resultMapping = assistant.buildResultMapping(
					...
					result.typeHandler() == UnknownTypeHandler.class ? null
							: result.typeHandler(), flags, null, null,
					isLazy(result));
