...
public class ProviderSqlSource implements SqlSource {
	...
	public ProviderSqlSource(Configuration config, Object provider) {
		...
		try {
			...
			this.providerType = (Class<?>) provider.getClass()
					.getMethod("type").invoke(provider);
			providerMethodName = (String) provider.getClass()
			...
			for (Method m : this.providerType.getMethods()) {
				if (providerMethodName.equals(m.getName())) {
					if (m.getParameterTypes().length < 2
						...
						this.providerTakesParameterObject = m
			...
			throw new BuilderException(
					"Error creating SqlSource for SqlProvider.  Cause: " + e, e);
		}
		if (this.providerMethod == null) {
			throw new BuilderException(
					"Error creating SqlSource for SqlProvider. Method '"
							+ providerMethodName
							+ "' not found in SqlProvider '"
		...
		try {
			...
			if (providerTakesParameterObject) {
				sql = (String) providerMethod.invoke(
						providerType.newInstance(), parameterObject);
			} else {
				sql = (String) providerMethod
						.invoke(providerType.newInstance());
			}
